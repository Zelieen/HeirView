...
class Slate:
    def __init__(self, width, height):
        ...
        self.__rootWidget.iconphoto(
            True, self.img
        )  # not sure if this is necessary after the above code line with "tk.call(...)"
    ...
    def draw_text(self, string, point, fill_color="black", size=24):
        self.canvas.create_text(
            (point.x, point.y), text=string, fill=fill_color, font="Lucida " + str(size)
        )
